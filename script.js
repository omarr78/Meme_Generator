 ...
 */
let globalRandomRiddle = -2;
const container1 = document.getElementById('proMemes');
const container2 = document.getElementById('joke');
const container3 = document.getElementById('quot');
const container4 = document.getElementById('rid');

function clearAll() {
    const arr = [container1, container2, container3, container4];
    arr.forEach(ele => ele.innerHTML = '');
    globalRandomRiddle = -2;
}
...
function showMeme() { 
    const randomMemeUrl = memes[rn(memes.length)];
...
function showJoke() {
    const randomJoke = jokes[rn(jokes.length)];
    const newText = document.createElement('p');
    newText.textContent = randomJoke;
    clearAll();
    container2.appendChild(newText);
}

function showQuote() {
    const randomQuoteQ = quotes[rn(quotes.length)].quote;
    const randomQuoteA = quotes[rn(quotes.length)].author;
    const newTextQ = document.createElement('p');
    const newTextA = document.createElement('p');
    newTextQ.textContent = randomQuoteQ;
    newTextA.textContent = "- "+ randomQuoteA;
    clearAll();
    container3.appendChild(newTextQ);
    container3.appendChild(newTextA);
}
...
function showRiddle() {
    clearAll();
    const num = rn(riddles.length);
    const randomRiddle = riddles[num].question;
    globalRandomRiddle = num;
    const newText = document.createElement('p');
    newText.textContent = randomRiddle;
    container4.appendChild(newText);
}
...
function revealAnswers() {
    if(globalRandomRiddle === -2){
        alert('there is no Riddle to answer it !!'); 
    }
    else if(globalRandomRiddle === -1)
    {
        alert('the answer is already exist !!');
    }
    else{
        const randomRiddle = riddles[globalRandomRiddle].answer;
        const newText = document.createElement('p');
        newText.textContent = "Answer : " + randomRiddle ;
        container4.appendChild(newText);
        globalRandomRiddle = -1;
    }
 }
